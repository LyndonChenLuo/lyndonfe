...
function configSigmaElements(config) {
    ...
    var a = [],
		b,x = 1;
		x_new = 0;
		areas = ['南亚', '西亚北非', '中亚', '东南亚', '中东欧', '独联体', '东北亚'];
		for (b in sigInst.clusters) a.push('<div style="line-height:12px"><a href="#' + b + '"><div style="width:40px;height:12px;border:1px solid #fff;background:' + b + ';display:inline-block"></div>'+ '  ' + areas[x_new++] + ' (' + sigInst.clusters[b].length + ' 个成员国)</a></div>');
    //a.sort();
...
function nodeActive(a) {
	...
	if (groupByDirection) {
		...
		f.push("<h2>相互链接的国家 (" + size + ")</h2>");
		(size>0)? f=f.concat(createList(mutual)) : f.push("暂无相互链接关系<br>");
		size=Object.size(incoming);
		f.push("<h2>纯链入国家 (" + size + ")</h2>");
		(size>0)? f=f.concat(createList(incoming)) : f.push("暂无纯链入关系<br>");
		size=Object.size(outgoing);
		f.push("<h2>纯链出国家 (" + size + ")</h2>");
		(size>0)? f=f.concat(createList(outgoing)) : f.push("暂无纯链出关系<br>");
	} else {
